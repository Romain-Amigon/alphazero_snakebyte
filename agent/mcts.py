# -*- coding: utf-8 -*-
"""
Created on Thu Mar 26 20:50:22 2026
...
def run_mcts(state, net, num_simulations, is_player_1=True):
    ...
    for _ in range(num_simulations):
        ...
        if is_terminal:
            ...
            with torch.no_grad():
                _, value_tensor = net(tensor_state)
            value = value_tensor.item()
            
            my_bots = sim_state.bots1 if is_player_1 else sim_state.bots2
            joint_actions = get_joint_actions(my_bots)
            
            prob_per_action = 1.0 / len(joint_actions) if len(joint_actions) > 0 else 1.0
            for ja in joint_actions:
                node.children[str(ja)] = Node(prob_per_action)
    ...
    best_action_str = max(action_visits, key=action_visits.get)
    return eval(best_action_str), action_visits